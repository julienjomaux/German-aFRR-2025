...
# -------------------------------
# TOP SECTION (kept exactly the same)
# -------------------------------
...
def plot_heatmap(data_up, data_dn, title, cbar_label, vmin, vmax,
    ...
    fig.text(
        ...
        fontsize=36, color='gray', alpha=0.3,
...
show_days_max = st.checkbox("Show day of maximal price instead of value", key="max_price_days")

if show_days_max:
    annot_up = build_max_day_annotation(df_up, upward_products,
                                        'GERMANY_AVERAGE_CAPACITY_PRICE_[(EUR/MW)/h]')
    annot_dn = build_max_day_annotation(df_dn, downward_products,
                                        'GERMANY_AVERAGE_CAPACITY_PRICE_[(EUR/MW)/h]')
else:
    annot_up = None
    annot_dn = None
...
show_days_marginal = st.checkbox("Show day of maximal marginal price instead of value",
